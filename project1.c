...
void movepawn(int xin[2],int yin[2],int rnd,int xcen[2],int ycen[2], int flag);
void init();
void display();
void dice(int );
int randm();
 ...
 */Button MyButton = {625,350, 100,30, 0,0, "Roll Dice", TheButtonCallback };
...
{
glColor3f(1,0,0);
midpoint(1000,800,2640,1);
midpoint(1000,2225,1240,-1);
...
{
	glColor3f(1,1,1);
	linebres(4200,2000,4200,2300);
	linebres(4200,2300,4500,2300);
	linebres(4500,2300,4500,2000);
	linebres(4500,2000,4200,2000);
...
void numbers(int xcen[2],int ycen[2],int random)
{
/*int i,j,count=0;
...
{
int direc,i,random;
int start[5][2]={{7*400,0},{6*400,400},{5*400,6*400},{400,400*9},{7*400,6*400}};
...
while(rnd--)
{
	...
	if(direc==1)
	{
		...
		{
		direc=0;
		translate(0,400,xcen,ycen,flag,random);
		yin[flag]+=400;
		}
		else
		{
		translate(400,0,xcen,ycen,flag,random);
		xin[flag]+=400;
		}
	}
	else
...
for(i=0;i<5;i++)
	if(xin[flag]==start[i][0]&&yin[flag]==start[i][1])
  		translate(end[i][0]-start[i][0],end[i][1]-start[i][1],xcen,ycen,flag,random);
...
void display()
{
int i,xcen[2]={5*400+300,5*400+300},ycen[2]={100,300},xin[2]={5*400,5*400},yin[2]={0,0},random;
//glClear(GL_COLOR_BUFFER_BIT);
Draw2D();
glColor3f(1,1,1);
//random=randm();
numbers(xcen,ycen,random);
/*random=randm();
dice(random);
//movepawn(xin,yin,1,xcen,ycen,0);
usleep(1000000);*/
