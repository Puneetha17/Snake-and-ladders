 ...
 */Button MyButton = {625,420, 50,25, 0,0, "Dice", TheButtonCallback };
...
void translate(int dx,int dy,int x1[2],int y1[2],int flag)
{
...
void display()
{
int i,xcen[2]={5*400+300,5*400+300},ycen[2]={100,300},xin[2]={5*400,5*400},yin[2]={0,0};
//glClear(GL_COLOR_BUFFER_BIT);
glColor3f(1,1,1);
for(i=1;i<=10;i++)
{
...
numbers(xcen,ycen);
//movepawn(xin[0],yin[0],3,xcen[0],ycen[0],0);
//dice(randm());
glFlush();
}
...
while(rnd--)
{
	...
	if(direc==1)
	{
		...
		{
		direc=0;
		translate(0,400,xcen,ycen,flag);
		yin[flag]+=400;
		}
		else
		{
		translate(400,0,xcen,ycen,flag);
		xin[flag]+=400;
		}
	}
	else
...
for(i=0;i<5;i++)
	if(xin[flag]==start[i][0]&&yin[flag]==start[i][1])
  		translate(end[i][0]-start[i][0],end[i][1]-start[i][1],xcen,ycen,flag);
